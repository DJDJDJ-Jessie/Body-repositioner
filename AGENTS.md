...
- 睡眠和锁屏是睡眠提醒生命周期的硬边界：进入睡眠/锁屏时必须关闭已有的睡眠提醒窗口并清除待播状态；恢复/解锁后重新开始提醒间隔，不得把睡眠或锁屏期间错过的提醒补播出来。
...
- `videos` 根目录是共享视频来源：当天专属/工作日/周末子文件夹被选中时，根目录中的直接视频必须与该子文件夹视频合并播放；没有匹配子文件夹时才只使用根目录。
